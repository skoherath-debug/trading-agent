...
_rw_1m_cache       = None
_rw_1m_cache_ts    = 0.0
_RW_CACHE_TTL      = 25   # was 45s — drop to match 30s scheduler for fresh data
...
# ── Option 1: Rocket/Waterfall SLOPE tracker ──────────────────────────────────
# Stores last 5 score samples with timestamps so we can compute rise rate
# and emit ROCKET_IMMINENT / WATERFALL_IMMINENT alerts before full ignition
_rw_score_history = []    # list of (timestamp, rocket, waterfall) tuples
_RW_HISTORY_MAX   = 5     # keep last 5 samples (= ~5 min at 60s tick)

# Thresholds for "imminent" prediction
_RW_IMMINENT_SCORE_MIN   = 45   # score must already be at least this high
_RW_IMMINENT_SLOPE_MIN   = 10   # points-per-minute rise rate to trigger
_RW_IMMINENT_ETA_MAX_SEC = 120  # only fire if expected ignition within 2min

# Separate cooldown for imminent alerts (shorter — they're forecasts, not entries)
_rw_imminent_alerted_dir  = "WAIT"
_rw_imminent_alerted_time = 0.0
_RW_IMMINENT_COOLDOWN     = 300   # 5min between imminent alerts same direction

def _calc_rocket_slope():
    """Return dict with rocket_slope, waterfall_slope (points per minute),
    plus ETA seconds until threshold hit, based on _rw_score_history."""
    if len(_rw_score_history) < 2:
        return {"rocket_slope": 0.0, "waterfall_slope": 0.0,
                "rocket_eta_sec": None, "waterfall_eta_sec": None,
                "samples": len(_rw_score_history)}

    # Use last 3 samples for slope (more stable than 2, more responsive than 5)
    recent = _rw_score_history[-3:] if len(_rw_score_history) >= 3 else _rw_score_history
    t0, r0, w0 = recent[0]
    t1, r1, w1 = recent[-1]
    dt_min = max((t1 - t0) / 60.0, 0.01)   # minutes, guard div0

    r_slope = (r1 - r0) / dt_min
    w_slope = (w1 - w0) / dt_min

    # ETA: seconds until score hits _RW_ENTRY_THRESHOLD (60) at current slope
    r_eta = None
    w_eta = None
    if r_slope > 0 and r1 < _RW_ENTRY_THRESHOLD:
        gap = _RW_ENTRY_THRESHOLD - r1
        r_eta = round(gap / r_slope * 60, 0)
    if w_slope > 0 and w1 < _RW_ENTRY_THRESHOLD:
        gap = _RW_ENTRY_THRESHOLD - w1
        w_eta = round(gap / w_slope * 60, 0)

    return {
        "rocket_slope":    round(r_slope, 1),
        "waterfall_slope": round(w_slope, 1),
        "rocket_eta_sec":    r_eta,
        "waterfall_eta_sec": w_eta,
        "samples": len(_rw_score_history),
    }

def _update_score_history(rocket, waterfall):
    """Append current scores and trim to last N samples."""
    import time as _t
    _rw_score_history.append((_t.time(), int(rocket), int(waterfall)))
    while len(_rw_score_history) > _RW_HISTORY_MAX:
        _rw_score_history.pop(0)

def _detect_imminent(rocket, waterfall, slope_data):
    """
    Option 1 logic: emit 'imminent' signal if score is rising fast enough
    to cross the entry threshold within _RW_IMMINENT_ETA_MAX_SEC.

    Returns one of: None, "ROCKET_IMMINENT", "WATERFALL_IMMINENT"
    """
    # Already past entry threshold — imminent logic doesn't apply
    if rocket >= _RW_ENTRY_THRESHOLD or waterfall >= _RW_ENTRY_THRESHOLD:
        return None

    r_slope = slope_data.get("rocket_slope", 0)
    w_slope = slope_data.get("waterfall_slope", 0)
    r_eta   = slope_data.get("rocket_eta_sec")
    w_eta   = slope_data.get("waterfall_eta_sec")

    # ROCKET imminent check
    if (rocket >= _RW_IMMINENT_SCORE_MIN and
        r_slope >= _RW_IMMINENT_SLOPE_MIN and
        r_eta is not None and r_eta <= _RW_IMMINENT_ETA_MAX_SEC and
        r_slope > w_slope):   # rocket must be rising faster than waterfall
        return "ROCKET_IMMINENT"

    # WATERFALL imminent check
    if (waterfall >= _RW_IMMINENT_SCORE_MIN and
        w_slope >= _RW_IMMINENT_SLOPE_MIN and
        w_eta is not None and w_eta <= _RW_IMMINENT_ETA_MAX_SEC and
        w_slope > r_slope):
        return "WATERFALL_IMMINENT"

    return None


# ── Option 2: Brain 4 — Rocket Predictor ML model ─────────────────────────────
# Trained separately in Colab on historical 1m bars. Target: "will rocket score
# hit 60+ within next 3 minutes?". 15 features drawn from current bar state.
# Falls back gracefully to disabled if model file missing.

_brain4_model    = None
_brain4_features = []
_brain4_loaded   = False
_BRAIN4_THRESHOLD = 0.60   # probability threshold for ROCKET_PREDICTED

def _load_brain4():
    """Load Brain 4 predictor model (called once at startup).
    Graceful no-op if files missing — Option 1 (slope) still works."""
    global _brain4_model, _brain4_features, _brain4_loaded
    if _brain4_loaded:
        return
    try:
        pkl_path  = "models/brain4_rocket_pred.pkl"
        json_path = "models/brain4_features.json"
        if not (os.path.exists(pkl_path) and os.path.exists(json_path)):
            print("[B4] Model files not found — Brain 4 predictor DISABLED")
            print("     (train via Colab notebook then upload .pkl to Google Drive)")
            _brain4_loaded = True   # mark as attempted
            return
        with open(pkl_path, "rb") as f:
            _brain4_model = pickle.load(f)
        with open(json_path) as f:
            cfg = json.load(f)
        _brain4_features = cfg.get("features", [])
        _brain4_loaded   = True
        print(f"[B4] Loaded {len(_brain4_features)} features, "
              f"threshold={cfg.get('threshold', _BRAIN4_THRESHOLD)}")
    except Exception as e:
        print(f"[B4] Load failed: {e} — predictor DISABLED")
        _brain4_loaded = True   # mark attempted so we don't retry every tick

def _build_brain4_features(df, live_price=None):
    """Build the 15-feature vector Brain 4 was trained on.
    MUST MATCH the training notebook exactly — same names, same order, same calculations.
    Returns a single-row DataFrame or None if not enough data."""
    ...
    rsi_slope3= float(rsi_s.iloc[-1] - rsi_s.iloc[-4])    # 3-bar slope
    rsi_slope5= float(rsi_s.iloc[-1] - rsi_s.iloc[-6])    # 5-bar slope

    # MACD histogram + accel
    ...
    hist_slope = float(hist.iloc[-1] - hist.iloc[-3])
    ...
    # Bollinger squeeze (low width = compressed = breakout imminent)
    bb_std  = close.rolling(20).std()
    bb_mid  = close.rolling(20).mean()
    bb_w_now  = float((4 * bb_std.iloc[-1]) / (bb_mid.iloc[-1] + 1e-9))
    bb_w_prev = float((4 * bb_std.iloc[-6]) / (bb_mid.iloc[-6] + 1e-9))
    bb_squeeze = bb_w_now / (bb_w_prev + 1e-9)   # <1 = squeezing
    ...
    body_size  = float(abs(close.iloc[-1] - openp.iloc[-1]) / (atr + 1e-9))

    # Momentum / trend
    mom_3 = float(close.iloc[-1] - close.iloc[-4])
    mom_5 = float(close.iloc[-1] - close.iloc[-6])
    price_vs_ema9  = (price - ema9_v) / (atr + 1e-9)
    price_vs_ema21 = (price - ema21_v) / (atr + 1e-9)
    ...
    feats = {
        ...
        "ema9_vs_ema21":   (ema9_v - ema21_v) / (atr + 1e-9),
        "ema21_vs_ema50":  (ema21_v - ema50_v) / (atr + 1e-9),
    }
...
def _run_brain4_prediction(df, live_price=None):
    """Run Brain 4 prediction. Returns dict with rocket_prob, waterfall_prob,
    or zeros if model not loaded."""
    if not _brain4_loaded:
        _load_brain4()

    if _brain4_model is None or not _brain4_features:
        return {"b4_rocket_prob": 0.0, "b4_waterfall_prob": 0.0,
                "b4_signal": None, "b4_loaded": False}

    try:
        X = _build_brain4_features(df, live_price)
        if X is None:
            return {"b4_rocket_prob": 0.0, "b4_waterfall_prob": 0.0,
                    "b4_signal": None, "b4_loaded": True, "b4_error": "not enough bars"}

        # Ensure column order matches training
        missing = [f for f in _brain4_features if f not in X.columns]
        if missing:
            return {"b4_rocket_prob": 0.0, "b4_waterfall_prob": 0.0,
                    "b4_signal": None, "b4_loaded": True,
                    "b4_error": f"missing features: {missing[:3]}"}
        X = X[_brain4_features]

        # Model is binary by default: class 1 = rocket fires in next 3m
        # If you trained a multiclass model, handle here
        if hasattr(_brain4_model, "predict_proba"):
            proba = _brain4_model.predict_proba(X)[0]
            # Assume proba[1] = rocket probability; if 3-class, adjust
            if len(proba) >= 2:
                rocket_prob    = float(proba[1])
                waterfall_prob = float(proba[2]) if len(proba) >= 3 else 0.0
            else:
                rocket_prob = float(proba[0])
                waterfall_prob = 0.0
        else:
            pred = int(_brain4_model.predict(X)[0])
            rocket_prob = 1.0 if pred == 1 else 0.0
            waterfall_prob = 1.0 if pred == 2 else 0.0

        signal = None
        if rocket_prob    >= _BRAIN4_THRESHOLD: signal = "ROCKET_PREDICTED"
        elif waterfall_prob >= _BRAIN4_THRESHOLD: signal = "WATERFALL_PREDICTED"

        return {
            "b4_rocket_prob":    round(rocket_prob, 3),
            "b4_waterfall_prob": round(waterfall_prob, 3),
            "b4_signal":         signal,
            "b4_loaded":         True,
        }
    except Exception as e:
        return {"b4_rocket_prob": 0.0, "b4_waterfall_prob": 0.0,
                "b4_signal": None, "b4_loaded": True, "b4_error": str(e)[:80]}
...
def run_rocket_analysis(live_price=None):
    ...
    # ── PHASE 31: Update history + compute slope + predict ─────────────────
    _update_score_history(rocket, waterfall)
    slope_data = _calc_rocket_slope()
    imminent   = _detect_imminent(rocket, waterfall, slope_data)

    # Brain 4 ML prediction (safe no-op if model not trained yet)
    b4 = _run_brain4_prediction(df1, live_price)
    ...
    return {
        ...
        # ── PHASE 31: Prediction fields ─────────────────────────────────
        "rocket_slope":     slope_data.get("rocket_slope", 0),
        "waterfall_slope":  slope_data.get("waterfall_slope", 0),
        "rocket_eta_sec":    slope_data.get("rocket_eta_sec"),
        "waterfall_eta_sec": slope_data.get("waterfall_eta_sec"),
        "slope_samples":    slope_data.get("samples", 0),
        "imminent":         imminent,            # ROCKET_IMMINENT / WATERFALL_IMMINENT / None
        "b4_rocket_prob":    b4.get("b4_rocket_prob", 0.0),
